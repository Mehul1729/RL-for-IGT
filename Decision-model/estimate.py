import numpy as np
import pandas as pd
from scipy.optimize import minimize
import matplotlib.pyplot as plt
import seaborn as sns
import wandb

# ============================================================
# 0. WANDB INITIALIZATION
# ============================================================

wandb.init(
    project="IGT-Arbitration-Model",
    name="DualSystem_FullInference",
    config={
        "optimizer": "L-BFGS-B",
        "n_actions": 4,
        "model": "Limbic-PFC Arbitration",
        "pfc_lr": 0.1,
        "pfc_gamma": 0.95
    }
)

# ============================================================
# 1. MODEL CLASSES
# ============================================================

class LimbicFit:
    def __init__(self, n_actions, alpha, decay, loss_aversion):
        self.n_actions = n_actions
        self.alpha = alpha
        self.decay = decay
        self.loss_aversion = loss_aversion
        self.q_table = np.zeros(n_actions)
        self.sensitivity = 0.5

    def pvl(self, reward):
        if reward >= 0:
            return np.power(reward, self.sensitivity)
        else:
            return -self.loss_aversion * np.power(abs(reward), self.sensitivity)

    def get_q(self):
        return self.q_table

    def update(self, action, reward):
        self.q_table = self.q_table * self.decay
        utility = self.pvl(reward)
        current_val = self.q_table[action]
        pe = utility - current_val
        self.q_table[action] = current_val + self.alpha * pe
        return abs(pe)

class PFCFit:
    def __init__(self, n_actions, lr=0.1, gamma=0.95):
        self.n_actions = n_actions
        self.lr = lr
        self.gamma = gamma
        self.q_table = {}

    def get_state_index(self, deck_counts):
        return tuple(np.array(deck_counts) // 5)

    def get_q(self, state):
        return np.array([self.q_table.get((state, a), 0.0) for a in range(self.n_actions)])

    def update(self, state, action, reward, next_state):
        old_val = self.q_table.get((state, action), 0.0)
        next_qs = [self.q_table.get((next_state, a), 0.0) for a in range(self.n_actions)]
        target = reward + self.gamma * max(next_qs)
        pe = target - old_val
        self.q_table[(state, action)] = old_val + self.lr * pe
        return abs(pe)

# ============================================================
# 2. NEGATIVE LOG LIKELIHOOD
# ============================================================

def calculate_nll(params, choices, rewards, n_actions=4, i=0):
    alpha, decay, lam, t_pfc, t_limbic, arb_lr = params

    if t_pfc < 0.01 or t_limbic < 0.01:
        return 1e9

    limbic = LimbicFit(n_actions, alpha, decay, lam)
    pfc = PFCFit(n_actions)

    rel_pfc, rel_limbic = 0.5, 0.5
    deck_counts = np.zeros(n_actions, dtype=int)
    total_nll = 0.0

    for choice, reward in zip(choices, rewards):
        action = int(choice)

        state_pfc = pfc.get_state_index(deck_counts)
        q_pfc = pfc.get_q(state_pfc)
        q_limbic = limbic.get_q()

        p_pfc = np.exp(q_pfc / t_pfc - np.max(q_pfc / t_pfc))
        p_pfc /= np.sum(p_pfc)

        p_limbic = np.exp(q_limbic / t_limbic - np.max(q_limbic / t_limbic))
        p_limbic /= np.sum(p_limbic)

        weights = np.exp(np.array([rel_pfc, rel_limbic]) * 5.0)
        weights /= np.sum(weights)
        beta_pfc = weights[0]

        final_probs = beta_pfc * p_pfc + (1 - beta_pfc) * p_limbic
        total_nll -= np.log(final_probs[action] + 1e-9)

        next_counts = deck_counts.copy()
        next_counts[action] += 1
        next_state_pfc = pfc.get_state_index(next_counts)

        pe_pfc = pfc.update(state_pfc, action, reward, next_state_pfc)
        pe_limbic = limbic.update(action, reward)

        rel_pfc += arb_lr * ((1 - np.clip(pe_pfc / 1350.0, 0, 1)) - rel_pfc)
        rel_limbic += arb_lr * ((1 - np.clip(pe_limbic / 100.0, 0, 1)) - rel_limbic)

        deck_counts = next_counts
        i += 1
        print(f"Trial {i}: Choice={action}, Reward={reward}, NLL={total_nll:.2f}")

    return total_nll

# ============================================================
# 3. FITTING PIPELINE WITH WANDB
# ============================================================

def process_and_fit(filepath, group_name):
    print(f"Processing {group_name}...")
    df = pd.read_csv(filepath)

    if df['deck'].min() == 1:
        df['deck'] -= 1

    df['net_reward'] = df['gain'] + df['loss']
    subjects = df['subjID'].unique()
    results = []

    for idx, sub in enumerate(subjects, 1):
        print(f"Fitting Subject {idx}/{len(subjects)} (ID: {sub})")

        sub_df = df[df['subjID'] == sub]
        choices = sub_df['deck'].values
        rewards = sub_df['net_reward'].values

        x0 = [0.1, 0.5, 1.0, 10.0, 2.0, 0.05]
        bounds = [
            (0.01, 1.0),
            (0.01, 1.0),
            (0.01, 5.0),
            (0.1, 100.0),
            (0.1, 20.0),
            (0.01, 0.5)
        ]

        res = minimize(
            calculate_nll,
            x0,
            args=(choices, rewards, 4),
            bounds=bounds,
            method="L-BFGS-B"
        )

        r = {
            "Subject": sub,
            "Group": group_name,
            "NLL": res.fun,
            "Alpha_Limbic": res.x[0],
            "Decay": res.x[1],
            "Lambda_Loss": res.x[2],
            "Temp_PFC": res.x[3],
            "Temp_Limbic": res.x[4],
            "Temp_Ratio": res.x[3] / res.x[4],
            "Arb_Rate": res.x[5]
        }

        # -------- WANDB SUBJECT LOG --------
        wandb.log({
            f"{group_name}/NLL": r["NLL"],
            f"{group_name}/Alpha_Limbic": r["Alpha_Limbic"],
            f"{group_name}/Decay": r["Decay"],
            f"{group_name}/Lambda_Loss": r["Lambda_Loss"],
            f"{group_name}/Temp_Ratio": r["Temp_Ratio"],
            f"{group_name}/Arb_Rate": r["Arb_Rate"]
        })

        results.append(r)

    df_res = pd.DataFrame(results)

    # -------- WANDB GROUP AGGREGATES --------
    wandb.log({
        f"{group_name}_Mean_Lambda": df_res["Lambda_Loss"].mean(),
        f"{group_name}_Mean_Decay": df_res["Decay"].mean(),
        f"{group_name}_Mean_TempRatio": df_res["Temp_Ratio"].mean(),
        f"{group_name}_Mean_ArbRate": df_res["Arb_Rate"].mean()
    })

    return df_res

# ============================================================
# 4. RUN ALL GROUPS
# ============================================================

df_amp = process_and_fit(
    r"C:\Desktop folder\IGT datasets\rawData\IGTdata_amphetamine.csv",
    "Amphetamine"
)

df_hc = process_and_fit(
    r"C:\Desktop folder\IGT datasets\rawData\IGTdata_healthy_control.csv",
    "Healthy"
)

df_her = process_and_fit(
    r"C:\Desktop folder\IGT datasets\rawData\IGTdata_heroin.csv",
    "Heroin"
)

all_results = pd.concat([df_amp, df_hc, df_her])
all_results.to_csv("IGT_Biomarkers_Estimated.csv", index=False)

print("\n--- BIOMARKER EXTRACTION COMPLETE ---")
print(all_results.groupby("Group")[["Lambda_Loss", "Decay", "Temp_Ratio", "Arb_Rate"]].mean())

wandb.finish()


# import numpy as np
# import pandas as pd
# from scipy.optimize import minimize
# import warnings

# # Suppress warnings
# warnings.filterwarnings("ignore")

# # --- 1. MODEL CLASSES ---

# class LimbicFit:
#     def __init__(self, n_actions, alpha, decay, loss_aversion):
#         self.n_actions = n_actions
#         self.alpha = alpha
#         self.decay = decay
#         self.loss_aversion = loss_aversion
#         self.q_table = np.zeros(n_actions) 
#         self.sensitivity = 0.5 

#     def pvl(self, reward):
#         if reward >= 0:
#             return np.power(reward, self.sensitivity)
#         else:
#             return -self.loss_aversion * np.power(abs(reward), self.sensitivity)

#     def get_q(self):
#         return self.q_table

#     def update(self, action, reward):
#         # 1. Decay all
#         self.q_table = self.q_table * self.decay
        
#         # 2. Update chosen
#         utility = self.pvl(reward)
#         current_val = self.q_table[action]
        
#         pe = utility - current_val
#         self.q_table[action] = current_val + self.alpha * pe
#         return abs(pe) 

# class PFCFit:
#     def __init__(self, n_actions, lr=0.1, gamma=0.95):
#         self.n_actions = n_actions
#         self.lr = lr
#         self.gamma = gamma
#         self.q_table = {} 

#     def get_state_index(self, deck_counts):
#         return tuple(np.array(deck_counts) // 5)

#     def get_q(self, state):
#         return np.array([self.q_table.get((state, a), 0.0) for a in range(self.n_actions)])

#     def update(self, state, action, reward, next_state):
#         old_val = self.q_table.get((state, action), 0.0)
#         next_qs = [self.q_table.get((next_state, a), 0.0) for a in range(self.n_actions)]
#         next_max = max(next_qs) if next_qs else 0.0
        
#         target = reward + self.gamma * next_max
#         pe = target - old_val
        
#         self.q_table[(state, action)] = old_val + self.lr * pe
#         return abs(pe) 

# # --- 2. THE INFERENCE ENGINE (FIXED) ---

# def calculate_nll(params, choices, rewards, n_actions=4):
#     alpha, decay, lam, t_pfc, t_limbic, arb_lr = params
    
#     # FIX: Return a large number, NOT a string, if bounds are violated
#     if t_pfc < 0.01 or t_limbic < 0.01: 
#         return 1e9 
    
#     limbic = LimbicFit(n_actions, alpha, decay, lam)
#     pfc = PFCFit(n_actions) 
    
#     rel_pfc = 0.5
#     rel_limbic = 0.5
    
#     deck_counts = np.zeros(n_actions, dtype=int)
#     total_nll = 0.0
    
#     # FIX: Loop runs silently to allow optimizer to work
#     for choice, reward in zip(choices, rewards):
#         action = int(choice) 
        
#         # 1. Get Values
#         state_pfc = pfc.get_state_index(deck_counts)
#         q_pfc = pfc.get_q(state_pfc)
#         q_limbic = limbic.get_q()
        
#         # 2. Softmax
#         pfc_logits = q_pfc / t_pfc
#         p_pfc = np.exp(pfc_logits - np.max(pfc_logits))
#         p_pfc /= np.sum(p_pfc)
        
#         limbic_logits = q_limbic / t_limbic
#         p_limbic = np.exp(limbic_logits - np.max(limbic_logits))
#         p_limbic /= np.sum(p_limbic)
        
#         # 3. Arbitration
#         rel_scores = np.array([rel_pfc, rel_limbic])
#         w_logits = rel_scores * 5.0
#         weights = np.exp(w_logits - np.max(w_logits))
#         weights /= np.sum(weights)
#         beta_pfc = weights[0]
        
#         # Mixture
#         final_probs = (beta_pfc * p_pfc) + ((1 - beta_pfc) * p_limbic)
        
#         # 4. Score
#         prob_chosen = final_probs[action]
#         total_nll -= np.log(prob_chosen + 1e-9) 
        
#         # 5. Learning
#         next_counts = deck_counts.copy()
#         next_counts[action] += 1
#         next_state_pfc = pfc.get_state_index(next_counts)
        
#         pe_pfc = pfc.update(state_pfc, action, reward, next_state_pfc)
#         pe_limbic = limbic.update(action, reward)
        
#         norm_pe_pfc = np.clip(pe_pfc / 1350.0, 0, 1)
#         norm_pe_limbic = np.clip(pe_limbic / 100.0, 0, 1)
        
#         rel_pfc += arb_lr * ((1.0 - norm_pe_pfc) - rel_pfc)
#         rel_limbic += arb_lr * ((1.0 - norm_pe_limbic) - rel_limbic)
        
#         deck_counts = next_counts
        
#     return total_nll

# # --- 3. DATA LOADING & EXECUTION ---

# def process_and_fit(filepath, group_name):
#     print(f"\n--- Processing {group_name} ---")
#     try:
#         df = pd.read_csv(filepath)
#     except FileNotFoundError:
#         print(f"File not found: {filepath}")
#         return pd.DataFrame()

#     # Preprocessing
#     if df['deck'].min() == 1:
#         df['deck'] = df['deck'] - 1
        
#     df['net_reward'] = df['gain'] + df['loss']
    
#     subjects = df['subjID'].unique()
#     results = []
    
#     print(f"Found {len(subjects)} subjects. Starting optimization...")
    
#     for i, sub in enumerate(subjects):
#         sub_data = df[df['subjID'] == sub]
#         choices = sub_data['deck'].values
#         rewards = sub_data['net_reward'].values
        
#         x0 = [0.1, 0.5, 1.0, 10.0, 2.0, 0.05]
#         bounds = [
#             (0.01, 1.0),   # alpha
#             (0.01, 1.0),   # decay
#             (0.01, 5.0),   # lambda
#             (0.1, 100.0),  # temp PFC
#             (0.1, 20.0),   # temp Limbic
#             (0.01, 0.5)    # arb_lr
#         ]
        
#         res = minimize(
#             calculate_nll, x0, 
#             args=(choices, rewards, 4),
#             bounds=bounds,
#             method='L-BFGS-B'
#         )
        
#         # Only print every 5th subject to keep console clean
#         if (i+1) % 5 == 0:
#             print(f"  Fitted Subject {i+1}/{len(subjects)} (ID: {sub}) - NLL: {res.fun:.2f}")
        
#         r = {'Subject': sub, 'Group': group_name, 'NLL': res.fun}
#         r['Alpha_Limbic'] = res.x[0]
#         r['Decay'] = res.x[1]
#         r['Lambda_Loss'] = res.x[2]
#         r['Temp_PFC'] = res.x[3]
#         r['Temp_Limbic'] = res.x[4]
#         r['Temp_Ratio'] = res.x[3] / res.x[4]
#         r['Arb_Rate'] = res.x[5]
#         results.append(r)
        
#     return pd.DataFrame(results)

# # Run for all 3 datasets
# # REPLACE THESE PATHS WITH YOUR ACTUAL FILE PATHS
# # e.g., r"C:\Users\...\IGTdata_amphetamine.csv"
# # # Run for all 3 datasets


# df_amp = process_and_fit(r"C:\Users\mehul\OneDrive\Desktop\IGT datasets\rawData\IGTdata_amphetamine.csv", 'Amphetamine')
# df_hc = process_and_fit(r"C:\Users\mehul\OneDrive\Desktop\IGT datasets\rawData\IGTdata_healthy_control.csv", 'Healthy')
# df_her = process_and_fit(r"C:\Users\mehul\OneDrive\Desktop\IGT datasets\rawData\IGTdata_heroin.csv", 'Heroin')

# # Combine and Save
# if not df_amp.empty and not df_hc.empty:
#     all_results = pd.concat([df_amp, df_hc, df_her])
#     all_results.to_csv('IGT_Biomarkers_Estimated.csv', index=False)

#     print("\n--- BIOMARKER EXTRACTION COMPLETE ---")
#     print(all_results.groupby('Group')[['Lambda_Loss', 'Decay', 'Temp_Ratio', 'Arb_Rate']].mean())




# import numpy as np
# import pandas as pd
# from scipy.optimize import minimize
# import sys
# import warnings

# # Suppress warnings
# warnings.filterwarnings("ignore")

# # --- 1. MODEL CLASSES ---

# class LimbicFit:
#     def __init__(self, n_actions, alpha, decay, loss_aversion):
#         self.n_actions = n_actions
#         self.alpha = alpha
#         self.decay = decay
#         self.loss_aversion = loss_aversion
#         self.q_table = np.zeros(n_actions) 
#         self.sensitivity = 0.5 

#     def pvl(self, reward):
#         if reward >= 0:
#             return np.power(reward, self.sensitivity)
#         else:
#             return -self.loss_aversion * np.power(abs(reward), self.sensitivity)

#     def update(self, action, reward):
#         # 1. Decay all
#         self.q_table = self.q_table * self.decay
        
#         # 2. Update chosen
#         utility = self.pvl(reward)
#         current_val = self.q_table[action]
        
#         pe = utility - current_val
#         self.q_table[action] = current_val + self.alpha * pe
#         return abs(pe) 

# class PFCFit:
#     def __init__(self, n_actions, lr=0.1, gamma=0.95):
#         self.n_actions = n_actions
#         self.lr = lr
#         self.gamma = gamma
#         self.q_table = {} 

#     def get_state_index(self, deck_counts):
#         return tuple(np.array(deck_counts) // 5)

#     def get_q(self, state):
#         return np.array([self.q_table.get((state, a), 0.0) for a in range(self.n_actions)])

#     def update(self, state, action, reward, next_state):
#         old_val = self.q_table.get((state, action), 0.0)
#         next_qs = [self.q_table.get((next_state, a), 0.0) for a in range(self.n_actions)]
#         next_max = max(next_qs) if next_qs else 0.0
        
#         target = reward + self.gamma * next_max
#         pe = target - old_val
        
#         self.q_table[(state, action)] = old_val + self.lr * pe
#         return abs(pe) 

# # --- 2. THE INFERENCE ENGINE ---

# def calculate_nll(params, choices, rewards, n_actions=4):
#     alpha, decay, lam, t_pfc, t_limbic, arb_lr = params
    
#     # Safety Check: Prevent division by zero or negative temp
#     if t_pfc < 0.01 or t_limbic < 0.01: return 1e9 
    
#     limbic = LimbicFit(n_actions, alpha, decay, lam)
#     pfc = PFCFit(n_actions) 
    
#     rel_pfc = 0.5
#     rel_limbic = 0.5
    
#     deck_counts = np.zeros(n_actions, dtype=int)
#     total_nll = 0.0
    
#     for choice, reward in zip(choices, rewards):
#         action = int(choice)
        
#         # Safety: Ensure action is valid
#         if action < 0 or action >= n_actions: continue
        
#         # 1. Get Values
#         state_pfc = pfc.get_state_index(deck_counts)
#         q_pfc = pfc.get_q(state_pfc)
#         q_limbic = limbic.q_table
        
#         # 2. Softmax
#         pfc_logits = q_pfc / t_pfc
#         p_pfc = np.exp(pfc_logits - np.max(pfc_logits))
#         p_pfc /= np.sum(p_pfc)
        
#         limbic_logits = q_limbic / t_limbic
#         p_limbic = np.exp(limbic_logits - np.max(limbic_logits))
#         p_limbic /= np.sum(p_limbic)
        
#         # 3. Arbitration
#         rel_scores = np.array([rel_pfc, rel_limbic])
#         w_logits = rel_scores * 5.0
#         weights = np.exp(w_logits - np.max(w_logits))
#         weights /= np.sum(weights)
#         beta_pfc = weights[0]
        
#         # Mixture
#         final_probs = (beta_pfc * p_pfc) + ((1 - beta_pfc) * p_limbic)
        
#         # 4. Score
#         prob_chosen = final_probs[action]
#         total_nll -= np.log(prob_chosen + 1e-9) 
        
#         # 5. Learning
#         next_counts = deck_counts.copy()
#         next_counts[action] += 1
#         next_state_pfc = pfc.get_state_index(next_counts)
        
#         pe_pfc = pfc.update(state_pfc, action, reward, next_state_pfc)
#         pe_limbic = limbic.update(action, reward)
        
#         norm_pe_pfc = np.clip(pe_pfc / 1350.0, 0, 1)
#         norm_pe_limbic = np.clip(pe_limbic / 100.0, 0, 1)
        
#         rel_pfc += arb_lr * ((1.0 - norm_pe_pfc) - rel_pfc)
#         rel_limbic += arb_lr * ((1.0 - norm_pe_limbic) - rel_limbic)
        
#         deck_counts = next_counts
        
#     return total_nll

# # --- 3. EXECUTION ---

# def process_group(filepath, group_name):
#     print(f"\n--- Processing {group_name} ---")
#     try:
#         df = pd.read_csv(filepath)
#     except FileNotFoundError:
#         print(f"❌ File not found: {filepath}")
#         return pd.DataFrame()

#     # Preprocessing: Map 'deck' to 0-3
#     if 'deck' in df.columns:
#         if df['deck'].min() == 1:
#             df['deck'] = df['deck'] - 1
#     else:
#         print("❌ Error: 'deck' column not found.")
#         return pd.DataFrame()
        
#     # Reward Calculation
#     if 'gain' in df.columns and 'loss' in df.columns:
#         df['net_reward'] = df['gain'] + df['loss']
#     else:
#         print("❌ Error: 'gain' or 'loss' columns missing.")
#         return pd.DataFrame()
    
#     subjects = df['subjID'].unique()
#     print(f"Found {len(subjects)} subjects.")
    
#     group_results = []
    
#     for i, sub in enumerate(subjects):
#         print(f"Fitting Subject {i+1}/{len(subjects)} (ID: {sub})...", end="", flush=True)
        
#         try:
#             sub_data = df[df['subjID'] == sub]
#             choices = sub_data['deck'].values
#             rewards = sub_data['net_reward'].values
            
#             x0 = [0.1, 0.5, 1.0, 10.0, 2.0, 0.05]
#             bounds = [
#                 (0.01, 1.0),   # alpha
#                 (0.01, 1.0),   # decay
#                 (0.01, 5.0),   # lambda
#                 (0.1, 100.0),  # temp PFC
#                 (0.1, 20.0),   # temp Limbic
#                 (0.01, 0.5)    # arb_lr
#             ]
            
#             res = minimize(
#                 calculate_nll, x0, 
#                 args=(choices, rewards, 4),
#                 bounds=bounds,
#                 method='L-BFGS-B'
#             )
            
#             # Save results REGARDLESS of strict success
#             params = res.x
#             r = {
#                 'Subject': sub, 
#                 'Group': group_name, 
#                 'NLL': res.fun,
#                 'Success': res.success, # Record if it converged or not
#                 'Alpha_Limbic': params[0],
#                 'Decay': params[1],
#                 'Loss_Aversion': params[2],
#                 'PFC_Temp': params[3],
#                 'Limbic_Temp': params[4],
#                 'Arbitration_Rate': params[5],
#                 'Temp_Ratio': params[3] / params[4]
#             }
#             group_results.append(r)
#             print(" Done.")
            
#         except Exception as e:
#             print(f" Failed ({e})")

#     return pd.DataFrame(group_results)

# # --- RUN ---
# # Update paths to match your folder structure exactly
# base_path = r"C:\Users\mehul\OneDrive\Desktop\IGT datasets\rawData"

# df_amp = process_group(r"C:\Users\mehul\OneDrive\Desktop\IGT datasets\rawData\IGTdata_amphetamine.csv", 'Amphetamine')
# df_hc = process_group(r"C:\Users\mehul\OneDrive\Desktop\IGT datasets\rawData\IGTdata_healthy_control.csv", 'Healthy')
# df_her = process_group(r"C:\Users\mehul\OneDrive\Desktop\IGT datasets\rawData\IGTdata_heroin.csv", 'Heroin')

# # Combine & Save
# all_results = pd.concat([df_amp, df_hc, df_her])
# if not all_results.empty:
#     all_results.to_csv('IGT_Biomarkers_Estimated.csv', index=False)
#     print("\n✅ SAVED: IGT_Biomarkers_Estimated.csv")
#     print("\n--- AVERAGE BIOMARKERS ---")
#     print(all_results.groupby('Group')[['Lambda_Loss', 'Decay', 'Temp_Ratio']].mean())
# else:
#     print("\n❌ No data processed.")